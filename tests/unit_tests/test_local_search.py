...
from metahopt.local_search import PollOrder, LocalSearch, LocalSearchState
...
def test_local_search_init_state():
    ...
    assert solver.init_state(12) == LocalSearchState(solver, 42, 12, 0, 0, 1, None)
...
def test_local_search_score_vectorized(mocker: MockerFixture):
    m_score_vectorized = mocker.patch.object(mod, "score_vectorized")

    solver = MyLocalSearch(score_func, vectorized=True, rng_seed=rng_seed)
    assert solver.score(state, polling_set) is m_score_vectorized.return_value
    m_score_vectorized.assert_called_once_with(score_func, polling_set, False, rng_seed)
    ...
    assert solver.score(state, polling_set) is m_score_vectorized.return_value
    m_score_vectorized.assert_called_once_with(score_func, polling_set, True, rng_seed)
...
def test_local_search_score_not_vectorized(mocker: MockerFixture):
    ...
    assert solver.score(state, polling_set) is m_score_solutions.return_value
...
def test_local_search_solve(mocker: MockerFixture):
    def score_func(x):
        return dict(sol0=3, sol1=1, sol2=2)[x]

    solver = MyLocalSearch(score_func, max_iter=1)
    end_state, stats = solver.solve("sol0")
